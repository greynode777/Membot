...
class FundamentalSignals:
    ...
    lp_clean: Optional[bool] = None  # статус LP по RugCheck: True/False/None
    bundle_detected: Optional[bool] = None  # похоже ли на бандл при запуске (Bitquery)
...
def score_fundamentals(s: FundamentalSignals) -> dict:
    ...
    # Статус ликвидности (LP) — RugCheck. Если не удалось определить (None),
    # фактор не участвует, как и с другими "необязательными" проверками.
    add_evaluable(s.lp_clean is True, "lp_locked", evaluable=s.lp_clean is not None)
    add_negative(s.lp_clean is False, "lp_risk")

    # Bundle detection (Bitquery) — похоже ли на скупку токена подставными
    # кошельками в первые секунды после запуска. Если не удалось
    # определить (None) — фактор не участвует, без штрафа.
    add_evaluable(s.bundle_detected is False, "no_bundle_detected",
                  evaluable=s.bundle_detected is not None)
    add_negative(s.bundle_detected is True, "bundle_detected_risk")
