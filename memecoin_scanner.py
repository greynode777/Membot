...
DEFAULT_CONFIG = {
    ...
    "observation_hours_max": 24,
    "tracking_retention_hours": 30,
    "rug_liquidity_drop_pct": 0.5,
    "rug_price_drop_pct": 0.7,
    "min_age_minutes": 10,
...
def send_rug_alert(mint, name, symbol, reason):
    """Короткое уведомление, когда токен, за которым следили, оказался рагпулом.
    Не отправляется как 'рекомендация' — просто чтобы видеть в Telegram,
    что происходит с отслеживаемыми токенами, и не гадать по логам."""
    text = (
        f"🚫 *Рагпул обнаружен*\n"
        f"{escape_mdv2(name)} \\(${escape_mdv2(symbol)}\\)\n"
        f"{escape_mdv2(reason)}\n"
        f"Mint: `{escape_mdv2(mint)}`\n\n"
        f"Рекомендация не отправлена, наблюдение прекращено\\."
    )
    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    payload = {
        "chat_id": TELEGRAM_CHAT_ID,
    ...
    try:
        resp = SESSION.post(url, data=payload, timeout=10)
        if resp.status_code != 200:
            logger.warning(f"[send_rug_alert] Telegram ошибка: {resp.text}")
    except Exception as e:
        logger.warning(f"[send_rug_alert] Ошибка: {e}")


def send_heartbeat(tracking, started_at_ms):
    ...
    text = (
        ...
        f"Рагпулов поймано за это время: {escape_mdv2(rugged_total)}"
    )
    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    payload = {
        "chat_id": TELEGRAM_CHAT_ID,
...
def update_tracking(entry, tracking):
    ...
    if not t["rugged"]:
        if t["peak_liquidity"] and liq < t["peak_liquidity"] * (1 - CONFIG["rug_liquidity_drop_pct"]):
            t["rugged"] = True
            t["rugged_reason"] = (
                f"Ликвидность упала с пика ${t['peak_liquidity']:,.0f} до ${liq:,.0f} "
                f"(-{(1 - liq / t['peak_liquidity']):.0%})"
            )
        elif t["peak_price"] and price < t["peak_price"] * (1 - CONFIG["rug_price_drop_pct"]):
            t["rugged"] = True
            t["rugged_reason"] = (
                f"Цена упала с пика ${t['peak_price']:.8f} до ${price:.8f} "
                f"(-{(1 - price / t['peak_price']):.0%})"
            )
...
def main():
    logger.info("Запуск сканера мемкоинов v2 (с отслеживанием выживаемости 8-24ч)...")
    ...
    send_heartbeat(tracking, started_at_ms)
    ...
    while True:
        ...
        for res in results:
            ...
            # Свежеобнаруженный рагпул — уведомляем один раз и завершаем наблюдение
            if t.get("rugged") and not t.get("rug_notified"):
                t["rug_notified"] = True
                t["recommended"] = True
                logger.warning(f"🚫 РАГПУЛ: {name} (${symbol}) {mint} — {t.get('rugged_reason')}")
                send_rug_alert(mint, name, symbol, t.get("rugged_reason", ""))
                continue
            ...
            if age_hours > CONFIG["observation_hours_max"]:
                logger.info(f"⌛ {name} (${symbol}) вышел за окно наблюдения ({age_hours:.1f}ч), рекомендация не даётся")
                t["recommended"] = True
                continue

            # В окне 8-24ч, ещё без вердикта — считаем финальный score
