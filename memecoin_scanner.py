...
DEFAULT_CONFIG = {
    ...
    "dev_wallet_sold_pct": 0.9,
    "dev_wallet_confirmation_minutes": 10,
...
def get_dev_wallet_token_balance(mint, dev_wallet_address):
    ...
    cached = cache_get(_dev_balance_cache, f"{mint}:{dev_wallet_address}", CONFIG["cache_ttl_seconds"])
    if cached is not None:
        return cached
...
def score_token(rugcheck, dexdata, tracking_entry):
    ...
    # --- Проверка на продажу разработчиком ---
    # Разработчик, слив свои токены, — сигнал не менее серьёзный, чем вывод
    # ликвидности из пула: он больше не заинтересован в проекте.
    if tracking_entry and tracking_entry.get("dev_sold"):
        return 0, [(100, tracking_entry.get("dev_sold_reason", "Разработчик продал токены"))], {"age_minutes": None}
...
def update_tracking(entry, tracking):
    ...
    if mint not in tracking:
        tracking[mint] = {
            ...
            "dev_wallet_initial_balance": None,
            "dev_wallet_peak_balance": None,
            "dev_wallet_sold_pending_since_ms": None,
            "dev_sold": False,
            "dev_sold_reason": None,
        }
    ...
    # --- Отслеживание баланса кошелька разработчика ---
    # НЕ ПРОВЕРЕНО НАПРЯМУЮ: RugCheck отдаёт адрес создателя токена под одним
    # из нескольких вероятных названий поля (см. get_rugcheck_report). Если
    # адрес не найден — просто пропускаем эту проверку, не считаем ошибкой.
    creator_address = rugcheck.get("creator_address")
    if creator_address and not t.get("dev_sold"):
        if t["dev_wallet_address"] is None:
            t["dev_wallet_address"] = creator_address

        dev_balance = get_dev_wallet_token_balance(mint, t["dev_wallet_address"])
        if dev_balance is not None:
            if t["dev_wallet_initial_balance"] is None:
                t["dev_wallet_initial_balance"] = dev_balance
            if t["dev_wallet_peak_balance"] is None or dev_balance > t["dev_wallet_peak_balance"]:
                t["dev_wallet_peak_balance"] = dev_balance

            peak = t["dev_wallet_peak_balance"]
            if peak and peak > 0:
                sold_pct = 1 - dev_balance / peak
                if sold_pct >= CONFIG["dev_wallet_sold_pct"]:
                    if t["dev_wallet_sold_pending_since_ms"] is None:
                        t["dev_wallet_sold_pending_since_ms"] = now_ms
                    pending_minutes = (now_ms - t["dev_wallet_sold_pending_since_ms"]) / 60000
                    if pending_minutes >= CONFIG["dev_wallet_confirmation_minutes"]:
                        t["dev_sold"] = True
                        t["dev_sold_reason"] = (
                            f"Разработчик продал {sold_pct:.0%} токенов "
                            f"(было {peak:,.0f}, осталось {dev_balance:,.0f})"
                        )
                else:
                    t["dev_wallet_sold_pending_since_ms"] = None
            t["dev_wallet_last_balance"] = dev_balance
...
def main():
    ...
    while True:
        ...
        for res in results:
            ...
            if t.get("dev_sold") and not t.get("rug_notified"):
                t["rug_notified"] = True
                t["recommended"] = True
                logger.warning(f"🚫 ДЕВ СЛИЛ: {name} (${symbol}) {mint} — {t.get('dev_sold_reason')}")
                rugs_since_last_report.append((name, symbol, t.get("dev_sold_reason", "")))
                continue
            ...
            # Впервые достигли этой контрольной точки — считаем score
            ...
            if score >= CONFIG["min_score_to_alert"]:
                logger.info(f"✅ {stars} Score {score}/100 — {name} (${symbol}), чекпоинт {checkpoint}ч (токену {age_hours:.1f}ч) — рекомендация")
