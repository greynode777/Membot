...
def _safe_text(value) -> str:
    ...
    (название/тикер токена с DexScreener) — без этого токен с символами
    вроде "<" в названии ломает парсинг HTML в Telegram, и сообщение
    зависает в бесконечном повторе отправки, блокируя остальные команды.
    Вызывать РОВНО ОДИН РАЗ, в момент первого получения значения из
    внешнего источника — при повторном экранировании уже сохранённого
    значения текст испортится (& станет &amp;amp;).
    """
    if not value:
        return ""
...
async def candidate_discovery_once():
    ...
    for item in items[:20]:
        ...
        name = _safe_text((pair.get("baseToken") or {}).get("name", ""))
...
async def confirmed_recheck_once():
    ...
    for entry in due:
        address = entry["address"]
        name = entry.get("name", address[:8])
        old_skip_streak = entry.get("skip_streak", 0)

        result, trends = await _score_fundamentals_for_address(address)
        if not result:
            log(f"[confirmed] {name}: нет данных, откладываю проверку.")
            await confirmed.update_after_check(
                address, entry["last_score"], entry["last_verdict"], old_skip_streak
            )
            continue
        ...
        # Считаем, сколько проверок ПОДРЯД токен остаётся Skip — если
        # слишком долго, незачем отслеживать его вечно.
        new_skip_streak = old_skip_streak + 1 if result["verdict"] == "Skip" else 0

        log(f"[confirmed] {name}: {old_score} -> {new_score} ({result['score_pct']}%) "
            f"({result['verdict']}, skip_streak={new_skip_streak}, "
            f"тренд ликв.={liquidity_trend}, тренд объёма={volume_trend}, "
            f"тренд devbal={dev_balance_trend})")

        if new_skip_streak >= config.CONFIRMED_AUTO_REMOVE_AFTER_SKIPS:
            await confirmed.remove_token(address)
            await telegram.send_message(
                f"🗑 <b>{name}</b> убран из 12ч-мониторинга — "
                f"остаётся Skip уже {new_skip_streak} проверки подряд."
            )
            log(f"[confirmed] {name}: убран из мониторинга (skip_streak={new_skip_streak}).")
            await asyncio.sleep(1.5)
            continue
        ...
        await confirmed.update_after_check(address, new_score, result["verdict"], new_skip_streak)
        await asyncio.sleep(1.5)  # небольшая пауза, чтобы не бить по лимитам DexScreener
...
async def build_token_card(address: str, name_hint: str = ""):
    ...
    name = _safe_text(base.get("name")) or name_hint or address[:8]
    symbol = _safe_text(base.get("symbol")) or "?"
...
async def render_compare_view(addr_a: str, addr_b: str) -> str:
    ...
    name_a = _safe_text((pair_a.get("baseToken") or {}).get("symbol")) or addr_a[:6] if pair_a else addr_a[:6]
    name_b = _safe_text((pair_b.get("baseToken") or {}).get("symbol")) or addr_b[:6] if pair_b else addr_b[:6]
    ...
    if text.startswith("/"):
        ...
        if command in ("/observed", "/наблюдение"):
            ...
            if not args or not is_valid_solana_address(args[0]):
                ...
                name = _safe_text((pair.get("baseToken") or {}).get("name", "")) if pair else ""
        ...
        elif command == "/unmonitor":
            if len(parts) < 2 or not is_valid_solana_address(parts[1]):
                await telegram.send_message("Использование: <code>/unmonitor АДРЕС_ТОКЕНА</code>")
            else:
                removed = await confirmed.remove_token(parts[1])
                await telegram.send_message(
                    "✅ Убрано из 12ч-мониторинга" if removed
                    else "ℹ️ Такого адреса нет под мониторингом"
                )
        else:
            await telegram.send_message(
