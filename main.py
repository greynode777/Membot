...
async def _score_fundamentals_for_address(address, manual_flags=None):
    ...
    # Баланс кошелька дева: сначала узнаём адрес (кэшируется навсегда,
    # RugCheck запрашивается только один раз на токен), затем текущий
    # баланс этого адреса по данному токену
    dev_wallet_address = await dev_wallets.get_dev_wallet(address)
    dev_balance = None
