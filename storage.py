...
async def record_snapshot(token_address, liquidity_usd, volume_24h_usd, price_usd):
    if db.is_configured():
        await db.run(
            "INSERT INTO snapshots (token_address, ts, liquidity_usd, volume_24h_usd, price_usd) "
            "VALUES (%s, %s, %s, %s, %s)",
            (token_address, time.time(), liquidity_usd, volume_24h_usd, price_usd),
        )
    else:
        _json_record_snapshot(token_address, liquidity_usd, volume_24h_usd, price_usd)
...
def _json_record_snapshot(token_address, liquidity_usd, volume_24h_usd, price_usd):
    data = _json_load()
