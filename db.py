...
async def init_schema():
    ...
    await run("""
        CREATE TABLE IF NOT EXISTS snapshots (
            ...
            dev_balance DOUBLE PRECISION
        )
    """)
    # На случай, если таблица уже существовала с прошлой версии — добавляем
    # колонку отдельно (CREATE TABLE выше её не добавит в уже созданную таблицу)
    await run("ALTER TABLE snapshots ADD COLUMN IF NOT EXISTS dev_balance DOUBLE PRECISION")
    ...
    """)
    await run("ALTER TABLE confirmed_tokens ADD COLUMN IF NOT EXISTS skip_streak INTEGER DEFAULT 0")
    ...
    await run("""
        CREATE TABLE IF NOT EXISTS dev_wallets (
            ...
            dev_wallet TEXT,
            lp_clean BOOLEAN,
            lp_checked_ts DOUBLE PRECISION
        )
    """)
    # На случай, если таблица уже существовала с прошлой версии
    await run("ALTER TABLE dev_wallets ADD COLUMN IF NOT EXISTS lp_clean BOOLEAN")
    await run("ALTER TABLE dev_wallets ADD COLUMN IF NOT EXISTS lp_checked_ts DOUBLE PRECISION")
    await run("ALTER TABLE dev_wallets ALTER COLUMN dev_wallet DROP NOT NULL")

    await run("""
        CREATE TABLE IF NOT EXISTS bundle_checks (
            address TEXT PRIMARY KEY,
            bundle_detected BOOLEAN NOT NULL
        )
    """)
    ...
    """)
    await run("CREATE INDEX IF NOT EXISTS idx_score_history_addr_ts ON score_history(token_address, ts)")
